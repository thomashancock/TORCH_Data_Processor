...
EventTreeManager::EventTreeManager(
	...
	const unsigned int nReadouts
) :
	RootManager(config,outfileName,"event_tree"),
	m_nReadouts(nReadouts),
...
void EventTreeManager::add(
	...
	for (const auto& packet : packets) {
		...
		b_eventID[b_nReadouts] = eventID;
		ASSERT(nullptr != b_bunchID);
		b_bunchID[b_nReadouts] = packet->getBunchID();
		ASSERT(nullptr != b_rocTime);
		b_rocTime[b_nReadouts] = packet->getROCValue();
		ASSERT(nullptr != b_nHeaders);
		b_nHeaders[b_nReadouts] = 1;
		ASSERT(nullptr != b_nLeadingEdges);
		b_nLeadingEdges[b_nReadouts] = packet->getNLeadingEdges();
		ASSERT(nullptr != b_nTrailingEdges);
		b_nTrailingEdges[b_nReadouts] = packet->getNTrailingEdges();
		ASSERT(nullptr != b_nTrailers);
		b_nTrailers[b_nReadouts] = 1;
		ASSERT(nullptr != b_wordCount);
		b_wordCount[b_nReadouts] = packet->getWordCount();

		// Add edges to total sum
		b_nEdges += b_nLeadingEdges[b_nReadouts];
		b_nEdges += b_nTrailingEdges[b_nReadouts];
		b_nReadouts++;
...
void EventTreeManager::setUpBranches() {
	...
	setupArrBranch<UInt_t>("rocTime", b_rocTime, "[nTDCs]/i", m_nReadouts);
	setupArrBranch<UInt_t>("nHeaders", b_nHeaders, "[nTDCs]/i", m_nReadouts);
	setupArrBranch<UInt_t>("nLeadingEdges", b_nLeadingEdges, "[nTDCs]/i", m_nReadouts);
	setupArrBranch<UInt_t>("nTrailingEdges", b_nTrailingEdges, "[nTDCs]/i", m_nReadouts);
	setupArrBranch<UInt_t>("nTrailers", b_nTrailers, "[nTDCs]/i", m_nReadouts);
	setupArrBranch<UInt_t>("wordCount", b_wordCount, "[nTDCs]/i", m_nReadouts);
